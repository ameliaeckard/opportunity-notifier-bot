...
def make_internship(index: int) -> Opportunity:
    return Opportunity(source="test", external_id=str(index), kind="internship", organization=f"Example Co {index}", title="Software Engineering Intern", location="Remote", url=f"https://example.com/job/{index}")
...
def test_hackathon_digest_uses_test_footer():
    item = Opportunity(source="test", external_id="2", kind="hackathon", organization="Hack Example", title="Example University", location="Charlotte, NC", url="https://example.com/event", start_date="2026-10-10T12:00:00Z", end_date="2026-10-12T12:00:00Z")
